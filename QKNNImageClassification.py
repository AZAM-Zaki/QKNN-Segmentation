...
TrainImage, TrainName = read_image(TrainPath)
TrainClass = ExtractName2Class(TrainName)
TrainRegions = ExtractProperties(TrainImage)

dataTrain = pd.DataFrame(TrainRegions)
# Using DataFrame.insert() to add a column
dataTrain.insert(0, "FileName",TrainName , False)
dataTrain.insert(4, "Class", TrainClass,False)

CalcCircularity(dataTrain)

TestImage, TestName = read_image(TestPath)
TestClass = ExtractName2Class(TestName)
TestRegions = ExtractProperties(TestImage)

dataTest = pd.DataFrame(TestRegions)
# Using DataFrame.insert() to add a column
dataTest.insert(0, "FileName",TestName , False)
dataTest.insert(4, "Class", TestClass, False)

CalcCircularity(dataTest)

#Preparing the dataset to pass through the QKNN
x_train = np.array([dataTrain['eccentricity'], dataTrain['Circularity']])
x_train = np.transpose(x_train)
x_valid = np.array([dataTest['eccentricity'], dataTest['Circularity']])
x_valid = np.transpose(x_valid)

train_label = np.array(dataTrain['Class'])
valid_label = np.array(dataTest['Class'])
...
Data = {'X':encoded_x_train[:,0], 'Y':encoded_x_train[:,1],
        'class':TrainClass  }
df_encoded = pd.DataFrame(data= Data)
sns.scatterplot(df_encoded, x='X', y='Y', hue = 'class')
plt.show() 

Data = {'X':encoded_x_valid[:,0], 'Y':encoded_x_valid[:,1],
        'class':TestClass  }
df_encoded = pd.DataFrame(data= Data)
sns.scatterplot(df_encoded, x='X', y='Y', hue = 'class')
plt.show() 
...
for ulang in range(Pengulangan):
    ...
    print("Nilai auto Circuit Result", AutoCircuitResult)
    print("Nilai Auto all counts", Auto_all_counts)
